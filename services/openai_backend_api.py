...
class OpenAIBackendAPI:
    ...
    def download_image_bytes(self, urls: list[str], timeout: float = 120.0) -> list[bytes]:
        images = []
        for url in urls:
    ...
    def stream_conversation(
        ...
        if "picture_v2" in system_hints:
            yield from self._stream_picture_conversation(prompt, model, images or [], deadline)
            return
    ...
    def _stream_picture_conversation(
        ...
        yield from self._iter_sse_payloads_capped(response, float(config.image_poll_timeout_secs), deadline)

    def _iter_sse_payloads_capped(self, response: Any, hard_cap_secs: float, deadline: float | None = None) -> Iterator[str]:
        """按墙钟硬上限消费图片 SSE 流，避免上游异常时长连接被无限挂起。

        curl_cffi 在 stream=True + 标量 timeout 下不限制流式 body 的总读取时长，
        上游未生成图片却保持连接时，读取会一直阻塞直到边缘重置（曾观测到单条流
        挂起约 29.5 分钟才失败）。这里使用「单请求总预算」的剩余时间作为硬上限：
        到点后关闭底层连接以解除阻塞，并抛出明确错误，让任务快速失败而非长时间挂起。
        """
        now = time.monotonic()
        if deadline is not None:
            ...
            if remaining <= 0:
                raise ImageStreamHardTimeoutError(
                    "图片生成流超时（单请求总预算已耗尽），已强制中断（上游可能未生成图片）"
                )
            cap = min(float(hard_cap_secs), remaining)
        else:
            cap = float(hard_cap_secs)
        local_deadline = time.monotonic() + cap
        # 看门狗：SSE 读取可能阻塞在底层 curl 调用中，超时后关闭连接以强制解除阻塞
        watchdog = threading.Timer(cap, response.close)
        watchdog.daemon = True
        watchdog.start()
        timeout_message = (
            f"图片生成流超时（已等待超过 {int(cap)} 秒硬上限），已强制中断（上游可能未生成图片）"
        )
        try:
            for payload in iter_sse_payloads(response):
                yield payload
